...
- Titles: cold-reader gate — `$AGENT_RULES_PATH/agents/editorial/titles.md`. Title + lede orient a stranger; no shape checklists. News outward / blog inward via `writing-news` / `writing-blog`. **Antora:** H1 = `nav.adoc` label = no divergent `:navtitle:` (Overview under a named section is the only exception) — `general/documentation.md`. In-repo IDE rules per `IDE_PROJECT_RULES` in `$HARNESS`; this file stays the paste preamble.
