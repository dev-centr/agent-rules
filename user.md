...
- **Audience / POV:** `page-*` attrs (Facto/`page-context`); keep `ifndef::page-context-active[]` fallback (teaching: audience / authors; **component homes: footer authors only — never show audience/usage chrome**). Agent-assisted **source** form: `<agent> on behalf of <human>` — **display** (page-context 0.6+): `Name (via Agent)`. Keywords link to `/home/keywords/{slug}/` listings. **Re-check after drafting** (no structural chat residue in openings; slug = H1 unless named absorb). Detail: `general/documentation.md`. Case study: docs hub `case-study-agent-voice-vs-reader-voice`.
