...
- Titles: `agents/editorial/titles.md` (canonical; not a per-site STYLE bible). **News = outward**; **blog = inward**. Cold-reader gate on title + lede; do not force `When …`. First-party news omits org; action essays pass implied [On]; prefer orientation shapes over rigid `X is Y`; attach modifiers to an object; docs topics = concept names. Philosophy: `Titles as orientation`. In-repo IDE rules per `IDE_PROJECT_RULES` in `$HARNESS`; this file stays the paste preamble.
