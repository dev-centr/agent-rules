...
- **Audience / POV:** `page-*` attrs (Facto/`page-context`); keep `ifndef::page-context-active[]` fallback (teaching: audience / authors; **component homes: footer authors only — never show audience/usage chrome**). Agent-assisted: `<agent> on behalf of <human>`. **Re-check after drafting** (no structural chat residue in openings; slug = H1 unless named absorb). Detail: `general/documentation.md`. Case study: docs hub `case-study-agent-voice-vs-reader-voice`.
