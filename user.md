...
- Titles: cold-reader gate — `$AGENT_RULES_PATH/agents/editorial/titles.md`. Title + lede orient a stranger; no shape checklists. News outward / blog inward via `writing-news` / `writing-blog`. **Antora:** H1 = `nav.adoc` label = no divergent `:navtitle:`; section landings = linked parent (link text = H1) + `index.adoc` + `html_extension_style: indexify` — never `.Section` + Overview, never `overview.adoc` / advertise `index.html` — `general/documentation.md`. In-repo IDE rules per `IDE_PROJECT_RULES` in `$HARNESS`; this file stays the paste preamble.
