...
**Hornet** was chosen as a working name because it echoes *harness* and because hornets *swarm* — parallel actor nodes. That metaphor fits **agents** better than the shell that hosts them; renaming the repo/runtime (for example to a plain `harness`) remains on the table.

=== Candidate: Jiminy Cricket / Cricket

The Pinocchio conscience on the shoulder — agents have no conscience of their own; something in the stack steers “how to be good.” Two placement options under discussion:
...
| **Harness / runtime**
| Conscience-as-shell: the product that sits with you and steers work. CLI/product length and Disney character association are drawbacks; plain `harness` stays stronger for discovery.

| **Model / advisory voice**
| Puts the **human in first person** (Pinocchio): you want to act; the model is the cricket on your shoulder. Stronger story for a chat/coordinator persona than for the binary that writes `graph.json`. Tension with Mixr: many models per swarm — brand one default advisory model, the coordinator voice, or a policy layer — not every routed worker.
|===

**Cricket** alone carries the metaphor with less character-name risk than the full *Jiminy Cricket* string.
