...
* Node graph persisted under `$CHAT_ROOT/`; spawn writes parent + child meta before accepting traffic
* Grid shows task nodes; fork panel loads `nodes/{id}/chat.jsonl`
* Coordinator thread stays thin; epochs freeze grid layout when coherence breaks
