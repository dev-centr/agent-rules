...
Desk UX: three primary projections over the same graph — **Linear** (discussion transcript), **Grid** (task tiles), **Tree** (spawn/dependency L→R) — plus **temporal scrubber** as an embeddable cross-cutting widget, not a fourth mode. Discussion and worker cadences stay separated (no worker noise in coordinator chat). Discussion nodes spawn task nodes; task tangents spawn discussion children.
...
* link:https://docs.devcentr.org/home/tools/agent-rules/[Agent rules portal]
