...
:page-audience: Dev-Centr contributors and new org members who need org policy stacked after portable `user.md`
:page-usage-context: Docs-hub explanation of the org overlay. Agent playbooks stay in `AGENTS.md` / `agents/*`
...
If the harness has **one undifferentiated** always-on field and cannot stack layers, compose personal + org with https://github.com/dev-centr/rules-manager[`rules-manager`] (org still after personal in the composed payload). Geography: xref:harness-neutral.adoc[Harness-neutral architecture].
