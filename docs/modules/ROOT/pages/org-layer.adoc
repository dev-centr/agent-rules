...
Rationale: one obvious place per org, one live canonical tree (`git pull`), no duplicate skill bodies. Initialize with `scripts/setup-org-agent-rules-wrapper.ps1` after `{org}/.github` exists. Skill `bootstrap-org` phase 3a.
