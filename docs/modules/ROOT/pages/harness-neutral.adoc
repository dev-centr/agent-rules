...
== DevCentr lightweight harness

DevCentr is building its own lightweight agent runtime (T3code-shaped weight, not Cursor-class IDE bulk) — wired to link:https://github.com/dlang-supplemental[dlang-supplemental] for native D infrastructure.

**Mixr** is the harness's model router meta-agent (Cursor *Auto*-shaped): it assigns models per node/job instead of pinning one default for the whole swarm.

See xref:agent-harness.adoc[Agent harness] for design direction, Mixr routing, OSS integration policy, and cross-org UX contracts (token provenance, actor-model UI from HCI Nerdz).
