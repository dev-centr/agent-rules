...
:description: How agent-rules separates forkable templates from machine-local harness config — polyglot AI assistants, HARNESS.md, MEMORIES.md, skills, and layer precedence.
...
* Portable baseline: `RULES.md`, `general/*`
* Org overlay: `AGENTS.md`, `agents/*`
* Canonical skill bodies: `skills/<name>/SKILL.md`
* Examples: `HARNESS.example.md`, `MEMORIES.example.md`, `profiles/my-desktop.md`
...
Facts about **one repository** → `<repo>/AGENTS.md` + docs. Not MEMORIES.
...
When `RULES.md` or `MAIN.md` is active, the agent reads **simultaneously**:
...
sequenceDiagram
  ...
  U->>A: Paste RULES.md preamble
  A->>M: Read HARNESS.md + MEMORIES.md
...
flowchart TD
  ...
  Q -->|Yes| W[Write HARNESS.md or MEMORIES.md or fork profile]
...
. Copy `MEMORIES.example.md` → `$CODE_ROOT/MEMORIES.md` as needed.
. Paste `RULES.md` (or rules-manager composed file) into always-on slot per `$HARNESS`.
...
* **`MEMORIES.md` is shared** across harnesses on the same machine (same tool paths).
