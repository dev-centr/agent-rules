...
Official `dub` has **no** `publish` command. When the user asks to **publish to dub / dlang**, use skill `publish-to-dub` (`skills/publish-to-dub/`) — `dubx` / `dub-publish`. Do not paste that skill body here.
