...
- **Hand-authored SVG / AsciiDoc encoding (agent, not Antora):** Antora copies `modules/*/images/` as-is; it does not generate those Access/GCP mock SVGs (Mermaid in the hub is client-side and separate). This is a **transcode** problem when it breaks, not a refactor. When you write `.svg` or `.adoc` on Windows:
  ...
  - Never “fix” encoding by re-saving mojibake (`â€œ` etc.) — that double-encodes.
  - **Run** skill `fix-docs-encoding` (`$AGENT_RULES_PATH/skills/fix-docs-encoding/`) — script `--check` / `--fix` on touched `docs/` before commit.
  - Mock chrome bars: one fill-only top-rounded `<path class="chrome">` + `<line class="chrome-rule">` below the breadcrumb; never a second **stroked** chrome `<rect>` through the label.
  - Hub deploy: pushing a component repo alone may not refresh docs.devcentr.org — bump/redeploy the **docs** hub playbook repo so `_images/` updates go live.
