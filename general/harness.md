...
- **Templates are forkable; machine files are not.** Never commit `$CODE_ROOT/HARNESS.md`, `$CODE_ROOT/MEMORIES.md`, or filled `profiles/<machine>.md` paths into upstream `dev-centr/agent-rules`.
...
- **Layer precedence is not automatic.** Harnesses do not rank portable vs org vs project rules — the agent must follow explicit stacking instructions in `AGENTS.md` / `RULES.md`.
...
2. `$CODE_ROOT/MEMORIES.md` (create from `MEMORIES.example.md` if missing)
