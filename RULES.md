...
- **Public README layout:** when creating/revising GitHub-facing READMEs, follow `general/readme-layout.md` (Best-README adapted: centered header, org **Docs \| Org** badge → hub URL, CI badge when CI exists, TOC if >3 sections, role-grouped Built With, back-to-top). Hand-edit per repo; blanks in `dev-centr/readme-template`.
