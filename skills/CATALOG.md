...
| `hive-watch` | hive-watch, hive remotes, HIVE_REMOTES_LAST_CHECKED, hive-watch.status.json, daily git fetch, avoid fetch every chat, install hive-watch | active | Tool: `dev-centr/hive-watch`; stamp in `machine.md` |
| `hive-layout` | hive layout, folder-schema, .forks placement, reconcile duplicate repos, misplaced fork at org root, bulk org sync, dedupe by origin URL, CODE_ROOT clone path | active | Fork vs owned via `gh api … .fork`; pairs with `general/folder-schema.md` |
...
| `antora-extension-consumers` | update Antora extension consumers, bump extension pins, discover dependents, GitHub topics for extensions, recursive consumer fan-out, `topic:antora-search-chat`, SemVer git pins | active | Topic = repo name (not `uses-*`); BFS fan-out |
...
| `web-crawlability` | public website/content route, sitemap.xml, robots.txt, crawler/indexing, SSR/SSG/prerender, empty app container, RSS/Atom | active | Meaningful raw HTML; generated discovery files; CI maintenance checks |
...
| `diagram-svg-output` | create/render/export/edit diagram, flowchart, architecture map, sequence diagram, Mermaid, PlantUML, Kroki, Graphviz, diagram SVG, themed SVG, dark/light diagram, web-compatible SVG | active | Runtime-inline host default; external adaptive + fixed/paired modes; semantic token presets |
