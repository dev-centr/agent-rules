...
5. **Extension consumers** — skill **`antora-extension-consumers`**: GitHub topic = upstream **repo name** (e.g. `antora-search-chat`, not `uses-*`); SemVer git pins; **recursive** fan-out after releases (intermediates like Valentus enqueue their own consumers when a published artifact moved).
