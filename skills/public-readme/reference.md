...
Canonical skill: `public-readme`. **GitHub URLs and `env-github` below are the default adapter.** Other forges: same sections; swap metric destinations (see `SKILL.md`).

Read this when creating or revising a public `README.md` / `README.adoc` for owned or org repos. Structure inspiration: [othneildrew/Best-README-Template](https://github.com/othneildrew/Best-README-Template). Canonical blanks: [dev-centr/readme-template](https://github.com/dev-centr/readme-template) (fork to other orgs / personal profile as needed).
